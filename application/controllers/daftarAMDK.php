...
class  daftarAMDK extends CI_Controller{

	function __construct(){
		parent::__construct();		
		$this->load->model('m_daftarAMDK');
	...
	function index(){
		...
		$this->load->view('v_daftarAMDK',$data);
	}
    ...
    $data = array(
		...
		'sistem_manajemen' => $this->input->post('sistem_manajemen'),
		...
		'gambarsuratijin' => $dataInfo[5]['file_name'],
		'gambarangka' => $dataInfo[6]['file_name'],
		'gambarsistem' => $dataInfo[7]['file_name'],
		'gambarijin' => $dataInfo[8]['file_name']
     );
	 $this->m_daftarAMDK->daftarAMDK($data,'data_perusahaan');
		redirect('login');
	}
